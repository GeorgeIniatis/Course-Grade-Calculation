...
def add_course(request):
    ...
    if (request.method == 'POST'):
        ...
        if course_form.is_valid():
            ...
            Permission.objects.create(codename='can_edit_course'+course_slug,name="can edit course "+course_slug,content_type=content_type,)
            Permission.objects.create(codename='can_upload_grades_for'+course_slug,name="can upload grades for "+course_slug,content_type=content_type,)
...
@login_required
@permission_required_context('chemapp.add_assessmentComponents','No permission to add_assessmentComponents',raise_exception=True)
...
def upload_course_csv(request):
    ...
    for column in csv.reader(io_string, delimiter=',', quotechar="|"):
        if not Course.objects.filter(code=column[0]).exists():
            ...
            degree.numberOfCourses = degree.numberOfCourses + 1
            degree.save()
            course_slug = column[0].upper()+"-"+column[1].upper()
            content_type = ContentType.objects.get_for_model(Course)
            Permission.objects.create(codename='can_edit_course'+course_slug,name="can edit course "+course_slug,content_type=content_type,)
            Permission.objects.create(codename='can_upload_grades_for'+course_slug,name="can upload grades for "+course_slug,content_type=content_type,)
